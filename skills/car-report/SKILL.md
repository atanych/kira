...
- [2026-05-20] [[car-report]] HARD RULE: при фактах про конкретные модели/цены/спецификации авто — НЕ угадывать. Открыть источник (WebFetch/agent-browser/Wikipedia), процитировать, привязать ссылку. Vovan 2026-05-19 поймал три ошибки подряд за один разговор (Zeekr 001 FR = Founder Edition $130k выдумала, история владельцев в РБ через отметки в техпаспорте — это РФ ПТС не РБ СРТС, портал.гаи.бел — выдуманный URL). Vovan сформулировал явно: 'приложи источники, чтоб я тебе верил'.
- [2026-05-20] [[car-report]] РБ ≠ РФ в авто-документах. РБ техпаспорт = СРТС (свидетельство о регистрации ТС), показывает ТОЛЬКО текущего владельца, никакого журнала. РФ ПТС с отметками о владельцах — это другая страна. История владельцев в РБ запрашивается через выписку из ГАИ: е-паслуга portal.gov.by, услуга 3.58.01 'Систематизированные сведения об автомобиле'. Параллельные варианты: mvd.gov.by (проверка на угон бесплатно), autoteka.by / avtocod.by / av.by (коммерческие, мгновенно).
- [2026-05-20] [[car-report]] Zeekr 001 FR/2024A в дилерском софте ZDS — это код рестайла на 800V (Future Road = маркетинговое название рестайл-версии с февр.2024), НЕ Founder Edition. Маркер архитектуры: pack voltage 714V = 800V платформа = рестайл; 400V = pre-restyle. FR Founder Edition = quad-motor лимитка 1264 hp (~$105k новой в Китае, не $130k — это я ошиблась), у неё 400V. Источник: en.wikipedia.org/wiki/Zeekr_001.
- [[2026-05-21]] [[car-report]] РБ авто перепродажа — обязательный пакет документов: ПТД (пассажирская таможенная декларация, остаётся у владельца, ГАИ берёт копию), квитанция утильсбора, ЭПТС (электронный, проверяется на elpts.ru по номеру, бесплатно), ТПО (таможенный приходный ордер — если НЕ льготный EV, подтверждает пошлину+НДС), СБКТС (свидетельство безопасности конструкции — обычно у дилера, физ.бумагу могут не отдать). Без ПТД или ТПО = серая схема, перепродать нельзя. Проверки по VIN: autoteka.by ($30-50), запрос в таможню через mns.gov.by. Если машина у дилера (Evolution Car итд) — оригиналы обычно остались у дилера, можно запросить.
- [[2026-05-22]] [[car-report]] EV wallbox на даче — однофазная сеть 32А = 7.4 кВт max (формула P=0.23×I для 230V/1ф, P=0.69×I/фаза для 400V/3ф). Чеклист для электрика: отдельный автомат C32, **УЗО тип B обязательно** (НЕ A — обычное УЗО типа A не срабатывает на DC-утечки от EV, это критично безопасно), кабель ВВГнг 3×6 мм², PE-заземление (в СТ часто без нормального контура — проверить отдельно). Бортовая AC Zeekr 001 — 11 кВт на 3ф, но в 1ф упирается в потолок 7.4 кВт. Wallbox: Pulsar Plus 7.4 (€700-800), Zaptec Go (дороже но проще монтаж), Teison/Feyree (€300-400 китайцы). Терминология: 'кВт в час' = бытовая ошибка; кВт = мощность, кВт·ч = энергия.
