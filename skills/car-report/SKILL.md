...
- [2026-05-14] [[car-report]] **Used market analysis для Vovan (бушные EV которые он рассматривает):** 🥇 Tesla Model Y LR AWD 2022 (до Tesla Vision рестайла, ультразвук есть) — лучший выбор, проблемы известные (phantom braking, touchscreen freezes, suspension knuckles, steering recall фев 2025), запчасти и know-how доступны. НЕ 2024 (худший reliability Consumer Reports за декаду), НЕ Performance (ещё жёстче подвеска), НЕ Standard Range RWD. Hard stops для семьи: жёсткая подвеска, phantom braking при детях, нет официала в РБ. 🥈 Xpeng G9 2023 — официал xpeng.by, но радар глючит, electromotor failure, скандал steering recall (Xpeng тайно менял дефекты без recall). 🥉 Zeekr 001 YOU 2024 — лучшее железо но power system failures массовые, battery recall фев 2026 (38к машин WE Edition thermal runaway), 6-мес refresh цикл фев→авг 2024 убил резидуал, владельцы 001 идут лечиться к Lynk & Co (свой сервис помойка).
- [2026-05-14] [[car-report]] **CIP цена ≠ итоговая для покупателя** при сделках через серых импортёров EV. CIP Minsk = Carriage and Insurance Paid (продавец оплачивает доставку+страховку до Минска). Покупателю всё равно: утильсбор (~$300), СБКТС (Свидетельство о безопасности конструкции ТС — обязательно для регистрации), ЭПТС (Электронный паспорт ТС, в базе ЕАЭС), постановка на учёт (~$300). Без льготы EV ещё пошлина 15% + НДС 20%. При сделке требовать смету включая ВСЕ эти позиции + кто их оформляет (импортёр или покупатель), на каком этапе деньги передаются.
- [2026-05-15] [[car-report]] **Новый проект projects/car-investment/** — бизнес-кейс на 1 авто/год (купить китайский EV, пригнать в Минск, продать). 1 авто/год = налогов нет (физлицо). Только BEV под льготу. Формула landed: USD = (цена_авто + $6300) × 1.015, затем BYN = landed_USD × курс + 750 BYN растаможка. $6300 = доставка Китай→Минск + оформление + страховка по словам поставщика #1 (Vovan подозревает скрытые платежи внутри, под оптимизацию). 1.5% = банк комиссия на (цена_авто + $6300). Файлы: README.md (статус, hard constraints), pricing.md (формула + калькулятор), suppliers.md (лог звонков). Не путать с projects/cars/ — там личный шортлист на замену XC90, тут перепродажа.
- [2026-05-16] [[car-report]] **GlobalCar.by / ООО 'Автопривоз ТриБро'** — серый импортёр авто в Минске. УНП 193714865, зарегистрирован 09.10.2023, юр.адрес Льва Толстого 8 оф.264а, директор Войтех Александр Николаевич, тел +375 44 744 99 99. **Red flag:** менеджер 'Дарья' имеет AI-сгенерённую аватарку (нулевой digital footprint, нет совпадений в reverse image search, идеальный портрет в стиле MJ/Flux). Перед сделкой — обязательно видеозвонок (AI-аватар не выдержит), договор от ООО с УНП, искать отзывы по юр.имени а не GlobalCar, контакт директора напрямую для крупных заказов.
- [2026-05-17] [[car-report]] Vovan планирует **навес для EV на даче СТ Восход-2012** (нет ни гаража ни навеса, есть парковка вдоль стены ~10м). План: пристенный навес-паровозик 10×3.5м под 2 машины подряд, профтруба 80×80 столбы (4 шт, шаг 3.3м) + 60×40 обрешётка, скат односкатный 25-30°, профлист (не поликарбонат — мутнеет за 3-5 лет), высота 2.7м снаружи / 3.2м у стены, водосток с двумя сливами, снегозадержатели. Бюджет: 1500-2500 USD местные / 3000-4000 фирмы. Зимняя защита — ПВХ-тенты по бокам (600-900 г/м², -30°C, на люверсах, $300-600). **ПВХ-тент НЕ держит тепло** (R-value 0, только ветрозащита/снегозащита) — но для EV не нужно, литий до -20°C нормально живёт, preheating через wallbox решает зимнюю просадку. Wallbox под навес у глубокой позиции (там EV стоит дольше). Без бетонного фундамента (винтовые сваи) — в СТ разрешение не требуется.
