...
- [2026-04-15] [[knowledge-base]] Tweet ingestion now uses x-thread (Apify, 200 replies) instead of FxTwitter. x-thread's discussion summary is used as the KB item summary — no extra KB AI summarization for tweets.
- [2026-04-21] [[knowledge-base]] YouTube ingest extractor uses **gemini-2.5-flash** (switched from gemini-2.0-flash after hitting free-tier daily quota on Tier 3). Cost ~$0.028/video. Planned fallback (approved by Vovan 2026-04-20, not yet implemented): Apify-based transcript actor `codepoetry` ($0.001/video, handles captionless videos via Whisper).
