...
- [[2026-07-22]] [[personal-tasks]] Расширение схемы 2026-07-21: колонки `scheduled_date DATE NULL` + `scheduled_time TIME NULL`. **Семантика:** `due_date` = дедлайн (до когда сделать), `scheduled_*` = событие (когда физически происходит — встреча, звонок). CLI: `add '...' --at '2026-07-22 11:00'` (с временем) / `--at 2026-07-22` (только день); `edit N --at '...'`; `--at none` очистить; `--due` для дедлайнов остался. В отчёте badge 🕐 (зелёный, событие) vs today/overdue/дата (due). **Утренний HTML + вечерний PNG** — две секции сверху: **🔥 НА СЕГОДНЯ** (overdue + сегодня, оранжевый accent) + **📅 НА ЗАВТРА** (синий, скрывается если пусто). Задачи из этих секций **НЕ дублируются** в тэг-группах ниже. Сортировка в 🔥: сначала со временем (09:00→18:00), потом overdue без времени, потом сегодня без времени.
