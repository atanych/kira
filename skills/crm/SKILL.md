...
- [2026-05-02] [[crm]] Recap script can attach wrong highlight — saw Product Reviews highlight bleed into oral/Vlad recap on 2026-05-01. When forwarding a recap, sanity-check that the highlight quote actually matches the meeting content; flag mismatches to Vovan instead of silently posting.
- [[2026-06-23]] [[crm]] 2026-06-22: managed DB-сокеты в `tmp/db/{database_url,crm_database_url}/.s.PGSQL.5432` принадлежат `volatclaw:bot_kira`. Если режим 755 (srwxr-xr-x) — bot_kira имеет только r-x, connect() падает с EACCES (cron action-items-reminder ловит это первым). Нужно 770 (srwxrwx---). Сам сокет править нельзя (owner=volatclaw), пишу Vovan'у — это починка на стороне платформы/брокера. После фикса cron оживает без рестарта.
